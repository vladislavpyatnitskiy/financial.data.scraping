...
rus.dividends.bar.plt <- function(x, s = NULL, e = NULL){ 
  ...
  for (j in 1:length(x)){ L <- NULL # Data for each security
    ...
    for (m in 1:length(y)){ a <- y[m] # Get data for each year
      ...
      if (isTRUE(any(l == v))){ # Data Frame of Dates and Dividends
        ...
        for (n in 0:(length(l) / 11)){ 
          
          if (isTRUE(l[(2 + n * 11)] == v)){
          
            D <- rbind.data.frame(
              D, 
              cbind(l[7 + n * 11], as.numeric(gsub(",", ".", l[4 + n * 11])))
            )
            }
          } 
        }
    ...
    for (n in 1:nrow(L)){ 
      ...
      while (isFALSE(L[n,1] == L[nrow(L),1])){
  ...
  for (n in 1:length(dates)){ 
    ...
    while (dates[n] != dates[length(dates)]){
...
rus.dividends.bar.plt(c("LKOH")) # Test
