...
finviz.sectors.dividends <- function(){ # Dividends Yield for each sector
  
  s <- read_html("https://finviz.com/groups.ashx?g=sector&v=110&o=name") %>%
    html_nodes('table') %>% .[[8]] %>% html_nodes('tr') %>% 
    html_nodes('td') %>% html_text() # Get data

  d <- data.frame(
    s[seq(from = 2, to = length(s), by = 14)], 
    s[seq(from = 5, to = length(s), by = 14)]
    )
